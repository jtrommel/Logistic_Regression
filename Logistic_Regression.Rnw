...
The response variable $y$ can only take the values ''0" and ''1". Or we should say the \emph{codes} ''0" and ''1". This coding is often based on another, continuous, variable $y'$. For example: the diameter of a steel rod can be influenced by the temperature ($x$) at which it is extruded. If the temperature is too low, the diameter will be too large. It is qualified as ''OK=1" when its diameter ($y'$) is below 25mm. \sidenote{The coding could of course be inverse with values above cut off get $y=0$, values below cut off get $y=1$}. When we call this cut off value for $y'$; $y'_{co}$ it follows that, and we assume that there is a linear relation between $y'$ and $x$ we get
\begin{equation}
  \begin{split}
    P[Y_{i}]=1=\pi_{i}&=P[Y'< y'_{co}] \\
...
With $\phi(z)$ the standard normal distribution this gives the \emph{probit mean response function}:
\begin{equation}
