...
In linear regression we determined the coefficients $\beta_{0}$ and $\beta_{1}$ using the \emph{Least Squares} criterium. However, because we start from one set $(x_{i}, y_{i})$ of experimental results, we cannot find the ''true" values of $\beta_{0}$ and $\beta_{1}$: we will find $b_{0}$ and $b_{1}$, one of many possible solutions. The same is true in logistical regression: based on the data of one experiment we will not find $\beta_{0}$ and $\beta_{1}$ but $b_{0}$ and $b_{1}$.

In linear regression the criterium for finding $b_{0}$ and $b_{1}$ was to find a \emph{minimum} of the error $\sum_{i=1}^{i=n}(y_{i}-\hat{y}_{i})^{2}$. An analytical solution of this problem is available. In logistical regression the criterium is to find the \emph{maximum} of the likelyhood function. No analytical solution is available and the maximum is found using numerical analysis.
