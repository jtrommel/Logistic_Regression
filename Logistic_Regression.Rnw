...
roc.plot(data.df[,2], preds, thresholds = seq(0, max(preds), 0.01), main="Receiver Operating Characteristic", xlab="False Positive Rate", ylab="True Positive Rate")
@
...
In this particular example we aim for a choice of the cut-off point that both maximises Q and minimises the False Negative Rate (FNR).
...
if (QandFNR$co[i] != 0 & QandFNR$co[i] !=1) {
...
ggplot(data=QandFNR) +
  ...
  labs(title="Influence of cut-off point on the Quality and the False Negative Rate", x="co", y="Q, FNR") +
  JT.theme
@

\begin{marginfigure}[0cm]
\includegraphics[width=1\textwidth]{LR-QandFNR}
\caption{Influence of the cut-off point on quality and False Negative Rate}
\label{fig:QandFNR}
\setfloatalignment{b}% forces caption to be bottom-aligned
\end{marginfigure}
