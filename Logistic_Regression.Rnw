...
This means that the mean response $E(Y_{i})=\beta_{0}+\beta_{1}x_{i}$ (the \emph{response function}) is no other than the probability that for this value of $x_{i}$ the response variable $Y_{i}$ will be equal to 1. 
...
But we know that all values of $E(Y_{i})$ should be ''0" or ''1" or in between those numbers. This is not always the case when $\beta_{0}$ and $\beta_{1}$ are determined using the least squares method.
...
\section{The ''sigmoid" or ''logistic" function}
