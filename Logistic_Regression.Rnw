...
<<>>=
modF <- glm(oring~temp,data=challenger,
            family=binomial(link="logit"))
tidymodF <- broom::tidy(modF)
tidymodF
@
...
ggplot(data=challenger, aes(x=temp, y=oring)) + 
  geom_point() + 
...
\begin{equation}
\begin{split}
D_{0}&=-2\ell_{0}=ln (\frac{1}{\mathcal{L_{0}}^{2})} \\
D_{a}&=-2\ell_{a}=ln (\frac{1}{\mathcal{L_{a}}^{2})}
\end{split}
\end{equation}
...
b0_hat <- tidymodF$estimate[1]
b1_hat <- tidymodF$estimate[2]
la <- sum(challenger$oring*(b0_hat + b1_hat*challenger$temp)) - sum(log(1 + exp(b0_hat + b1_hat*challenger$temp)))
...
<<>>=
library(aod)
wald.test(b=coef(modF),Sigma=vcov(modF),Terms=1)
wald.test(b=coef(modF),Sigma=vcov(modF),Terms=2)
@
...
\begin{itemize}
  \item quadrant I: $Y(x_{i})$ is predicted ''1" and is in reality ''1"
  \item quadrant II: $Y(x_{i})$ is predicted ''1" and is in reality ''0"
  \item quadrant III: $Y(x_{i})$ is predicted ''0" and is in reality ''1"
  \item quadrant IV: $Y(x_{i})$ is predicted ''0" and is in reality ''0"
\end{itemize}
