...
% with $P[Y=1]=\pi$ and $P[Y=0]=1-\pi$. We do not know all values of $Y$, but we have a sample (size=$n$) of $n$ pairs $(x_{i}, y_{i})$. However, the probability $\pi$ could be different for different values of $x_{i}$:
% 
% \begin{equation}
...
When we use a \emph{univariate linear regression} we take it that the relation between $Y$ and $x$ can be written as:
...
But we know that all values of $E(Y_{i})=\beta_{0} + \beta_{1}x_{i}$ should be ''0" or ''1" or in between those numbers. This is not always the case when $\beta_{0}$ and $\beta_{1}$ are determined using the least squares method.
