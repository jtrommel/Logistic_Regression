...
When the response variable $Y$ is \emph{qualitative} rather than \emph{quantitative}, linear regression is not the best method to link $Y$ with the controllable factors $x_{i}$\sidenote{Y (in capital) is a probability variable, x (lower case) is known}. 
...
The error term $\epsilon_{i}$ arises because other, unknown of uncontrollable, variables cause the pairs $(x_{i}, Y_{i})$ not to lie on the theoretical regression line. Because we assume that these errors are $N(0,\sigma)$ distributed it follows that
\begin{equation} 
\label{eq:linear}
...
This means that the mean response $E(Y_{i})=\beta_{0}+\beta_{1}x_{i}$ (the \emph{response function}) is no other than the probability that for this value of $x_{i}$ the response variable $Y_{i}$ will be equal to 1. Because probabilities are limited to the interval $[0,1]$, the response function has to be capped at the low and the high end (Figure~\ref{fig:linresp}).
...
The independent variable $x$ is continuous. To translate the value of $x$ into a binary decision $y$ we need to have some cut off point: values of $x$ above the cut off point will get $y=1$, while values of $x$ below the cut off will result in $y=0$\sidenote{The coding could of cours be inverse: values above cut off get $y=0$, values below cut off get $y=1$}. When we call this cut off value $x_{co}$ it follows that:
\begin{equation}
  \begin{split}
    P[Y_{i}]=1=\pi_{i}&=P[x>x_{co}] \\
    &=P[\beta_0 + \beta_{1}x_{i}+\epsilon{i} > \beta_0 + \beta_1x_{co}] \\
    &=P[\epsilon{i}>\beta_{1}(x_{co}-x_{i})] \\
    &=P[\frac{\epsilon -0}{\sigma}] > \frac{\beta_{1}}{\sigma}(x_{co}-x_{i}) \quad with \quad \epsilon \sim \mathcal{N} (0,\sigma) \\
    &=P \left[ Z> \left(  \frac{\beta_{1}x_{co}}{\sigma} - \frac{\beta_{1}x_{i}}{\sigma} \right) \right] \\
    &=P[Z>(\beta_{0}^{*}+\beta_{1}^{*}x_{i})]
  \end{split}  
\end{equation}

With $\phi(z)$ the standard normal distribution it follows that:
\begin{equation}
E(Y_{i})=\pi_{i}=1-\phi\left[ \beta_{0}^{*}+\beta_{1}^{*}x_{i})  \right]
\end{equation}
...
But this is a crude approximation. An \emph{S-shape} or \emph{sigmoid} seems more appropriate (see Figure~\ref{fig:intro}). This function is given by:
\begin{equation}
...
In linear regression the criterium for finding $b_{0}$ and $b_{1}$ was to find, using the Ordinary Least Squares method, a \emph{minimum} of the error which is the sum of the squares of the residuals:
\begin{equation}
...
When we accept  hypothesis 1 ($Y_{i}$ is the result of a Bernouilli experiment with probability $\pi_{i}$), then the probability distribution for $Y_{i}$ can be written as:
\begin{equation}
...
\begin{equation}
  ...
  \end{split}
\end{equation}
Because all probabilities lie between $0$ and $1$, it follows that the product of these probabilities will also have a result between these two limits: $0 \leq \mathcal{L} \leq 1$.
...
The log-likelihood function will have values between $-\infty$ and $0$.

In logistical regression the criterium is to find the \emph{maximum} of the likelyhood function. No analytical solution is available and the maximum is found using numerical analysis.
