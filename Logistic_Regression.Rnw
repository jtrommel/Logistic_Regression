...
The model has no value whatsoever when $\beta_{1}=0$. In that case the probability $\pi(x_{i})$ will be independent of $x_{i}$. When we rework the model with the restriction that $\beta_{1}=0$ the \emph{Likelihood function} and the \emph{Log Likelihood function} are given by:

\begin{equation}
\begin{split}
    \mathcal{L} &=\prod_{i=1}^{i=n}\frac{e^{y_{i}b_0}}{1 + e^{b_0}} \\
    \ell &= \sum_{i=1}^{i=n} \left[y_{i}b_{0} - ln(1+e^{b0} \right]
\end{split}
\end{equation}

These functions in $b_{0}$ have a maximum when
\begin{equation}
\frac{d}{db_{0}}\ell = \sum_{i=1}^{i=n}y_{i}- \sum_{i=1}^{i=n}\frac{e^{b_{0}}}{1+e^{b_{0}}}=\sum_{i=1}^{i=n}y_{i} - n\frac{e^{b_{0}}}{1+e^{b_{0}}}=0
\end{equation}
...
Now we can calculate the value of the \emph{Likelihood function} for the case with $\beta_{1}=0$ (this value we call $\mathcal{L}_{0}$) and for the case with the values for $\beta_{0}$ and $\beta_{1}$ given by the logit regression model (this value we call $\mathcal{L}_{a}$).

\chapter{An example}

\section{O-ring failure}

In the ''mcsm"-package data are given relating to the accident with the Challenger spacecraft, January 1986. O-rings were used to seal compartiments, but it was known that the failure of these rings depended on the temperature. 
...
The data are given by the following scatterplot (Figure~\ref{fig:scatter}). ''Failure" is coded as ''1".

We find the values for $b_{0}$ and $b_{1}$ for a logistic regression with the \emph{logit} function as link function using the \textit{glm}-function in R. The \emph{logit} function is superimposed on the data in Figure~\ref{fig:scatterplusmodel}.

<<>>=
modF <- glm(oring~temp,data=challenger,family=binomial(link="logit"))
broom::tidy(modF)
@
