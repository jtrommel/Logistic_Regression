...
\newpage
We can see look at the \emph{Receiver Operating Characteristic} (Figure~\ref{fig:ROC}). The ROC is a graph of the \emph{Sensitivity=True Positive Ratio=TPR} as a function of the \emph{Aspecificity=False Positive Ratio =FPR} for different values of the cut-off point. 

A perfect classifier would have a TPR=1 and a FPR=0. That is the point (0,1) in the graph.
If our model would simply guess for each temperature whether an o-ring is defective (p=0.5) we would roughly be right 50\% of the time (TPR=0.5) and wrong also 50\% of the time (FPR=0.5). As the sample size increases we would get closer and closer to this point. This is the point halfway on the diagonal.
Classifier models with different cut-off points will result into one ''model"-point in the graph. All point above the diagonal have a TPR>FPR which is good. Points below the diagonal represent classifier models with TPR<FPR. In those cases it is necessary to look at our choice of what we call ''good" and what we cal ''bad". The ROC-curve gives us the coordinates of the classifier model for different values of the cut-off point (Figure~\ref{fig:ROC}). All point lie above the diagonal, which is good. But most are far away from the ideal classifier.
