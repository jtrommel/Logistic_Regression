...
Our model (in this case the logistic regression model) should perform better than any blind classifier. A perfect classifier would predict all actual results correctly. This gives Table~\ref{tab:co05} with $TPR=1$ and $FPR=0$ (Point ''F" in Figure~\ref{fig:ROC2}).
...
His counterpart is the really obtuse classifier who gets prediction wrong: for every actual ''1" he predicts ''FALSE" and for every actual ''0" he predicts ''TRUE". That is represented by point G (1,0). In this case you could argue that this classifier is actually perfect, but it has misunderstood the interpretation of ''FALSE" and ''TRUE".

Our logistic regression classifier will give different confusion matrices for different choices of the cut-off points. Every choice will have a TPR and an FPR and thus a ''model"-point in the graph. A \emph{low} cut-off point will lead to a lot of ''TRUE"-predictions, which is equivalent to a blind classifier with a high p-value. We expect to find the coordinates of models with a \emph{low} cut-off point in the top right hand corner of the graph. Models with a \emph{high} cut-off point should fall into the lower left corner of the graph.Points above the diagonal have a TPR>FPR which is good. Points below the diagonal represent classifier models with TPR<FPR. In those cases it is necessary to look at our choice of what we call ''good" and what we call ''bad". 

The ROC-curve of our logistical regression model for different values of the cut-off point is given in Figure~\ref{fig:ROC2}. All points lie above the diagonal, which is good. But most are far away from the ideal classifier.
