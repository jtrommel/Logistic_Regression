...
\begin{enumerate}
  ...
  \item Although we started this example looking only at the cut off point $y'_{co}=25mm$ below which $Y_{i}=1$. In Figure~\ref{fig:probit} we can see that automatically there is also a cut off point below which $Y_{i}=0$.
  ...
  \item If we had used the opposite coding (''0" when $y'<y'_{co}$) we could call this probability variable $Y^{opp}_{i}$ and $Y^{opp}_{i}=1-Y$. But because $\phi(z)=1-\phi(-z)$ it follows that $P[Y^{opp}_{i}]= P[Y=0] = 1-\phi(\beta_{0}^{*}+\beta_{1}^{*}x)\phi(-\beta_{0}^{*}-\beta_{1}^{*}x)$. It has the same form, only the sign of the $\beta$'s has changed.
\end{enumerate}
