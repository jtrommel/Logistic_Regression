...
It is clear that the choice of the \emph{cut-off} point is of major influence. We can measure the effect of different values of the \emph{cut-off} point, by calculating the proportion of predictions, this is
\begin{equation}
Q = \frac{TP + TN}{n}
\end{equation}

We are trying to get $Q$ as high as possible.

Figure~\ref{fig:coinfluence} shows what happens to $Q$ when we choose different values for the cut-off point.

<<label=coinfluence,fig=TRUE,include=FALSE, echo=FALSE>>=
co_influence <- data.frame(co=seq(0.1, 0.9, length.out = 100), Q=0)
for (i in seq(1:100)){
co_influence$Q[i] <- (table(data.df[,2],predict(mod.lr, type="response")>co_influence$co[i])[1,1] + table(data.df[,2],predict(mod.lr, type="response")>co_influence$co[i])[2,2])/n 
}
ggplot(data=co_influence) +
  geom_line(aes(x=co, y=Q)) +
  labs(title="Influence of cut-off point on the\nproportion of correct predictions", x="co", y="Q") +
  JT.theme
@

\begin{marginfigure}[0cm]
\includegraphics[width=1\textwidth]{LR-coinfluence}
\caption{Influence of the cut-off point on the proportion of correct predictions}
\label{fig:coinfluence}
\setfloatalignment{b}% forces caption to be bottom-aligned
\end{marginfigure}

It seems that a cut-off value around 0.5 does indeed give the highest proportion of correct predictions. However, in view of the fact that the consequence of a failing o-ring could be (and in reality were) disastrous, we clearly do not want a model that leads to a classification where we predict ''Safe" (which in this case is coded as ''0"), while the reality is that it ''Fails" (coded as a ''1"). We do not want a False Negative. A False Positive means that the model classifies the situation as ''Fails" (coded as ''1"), while the reality is ''Safe" (coded as ''0"). This is annoying (the launch is delayed until the temperature is higher) and it will cost money, but these consequences are incomparable with the loss of life. We should therefore choose $co$ to minimize the False Negatives, even at the cost of higher False Positives. 

<<label=coinfluenceFN,fig=TRUE,include=FALSE, echo=FALSE>>=
co_influence <- data.frame(co=seq(0.1, 0.9, length.out = 100), FN=0)
for (i in seq(1:100)){
co_influence$FN[i] <- (table(data.df[,2],predict(mod.lr, type="response")>co_influence$co[i])[2,1])/n 
}
ggplot(data=co_influence) +
  geom_line(aes(x=co, y=FN)) +
  labs(title="Influence of cut-off point on the\nproportion of False Negatives", x="co", y="FN") +
  JT.theme
@
\begin{marginfigure}[0cm]
\includegraphics[width=1\textwidth]{LR-coinfluenceFN}
\caption{Influence of the cut-off point on the proportion of False Negatives}
\label{fig:coinfluenceFN}
\setfloatalignment{b}% forces caption to be bottom-aligned
\end{marginfigure}

Low values of the cut-off point (e.g. $co=0.2$) seem to be more appropriate in view of the catastrophical consequences of a False Negative. 
<<echo=FALSE>>=
co <- 0.2
comp.lr <- data.frame(pred=as.numeric(predict(mod.lr, type="response")>co), obs=data.df[,2], group.name=("TP"), stringsAsFactors=FALSE)
for (i in (1:nrow(comp.lr))) {
...
FALSE & FN=$\Sexpr{FN}$ & TN=$\Sexpr{TN}$ &
\end{tabular}
\caption{Confusion matrix; cut-off=0.2}
\label{tab:co02}
\end{margintable}

In this case the confusion matrix is given in Table~\ref{tab:co02}: only one False Negative but with a lower quality of our classification: $Q=\Sexpr{round(Q,3)}$.

\newpage
We can see this is a \emph{Receiver Operating Characteristic} (Figure~\ref{fig:ROC}).

<<label=ROC,fig=TRUE,include=FALSE, echo=FALSE>>=
co_influence <- data.frame(co=seq(0.1, 0.9, by=0.1), FP=0, TP=0)
for (i in seq(1:nrow(co_influence))){
co_influence$FP[i] <- (table(data.df[,2],predict(mod.lr, type="response")>co_influence$co[i])[1,2])/n
co_influence$TP[i] <- (table(data.df[,2],predict(mod.lr, type="response")>co_influence$co[i])[2,2])/n
}
ggplot(data=co_influence) +
