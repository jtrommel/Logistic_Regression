...
The parameter $\beta_{0}$ is a \emph{place} factor (Figure~\ref{fig:place_and_from}-left): for positive values of $\beta_{0}$ it shifts the graph to the left, for negative values to the right. Parameter $\beta_{1}$ is a \emph{form} factor (Figure~\ref{fig:place_and_from}-right): for high values of $\beta_{1}$ it squeezes the graph to the center, for low values it streches the graph. Negative values of $\beta_{1}$ makes the sigmoid function go from high (1) for negative values of $x$ to low (0) for positive values of $x$.

<<label=place_and_form,fig=TRUE,include=FALSE, echo=FALSE>>=
df_pandf <- data.frame(x=seq(-10,10,0.5),y1=0, y2=0, y3=0, y4=0, y5=0, y6=0)
...
p1 <- ggplot(data=df_pandf, aes(x=x)) + 
  ...
  labs(title="beta0 changes place", xlab="x", ylab="y") +
...
p2 <- ggplot(data=df_pandf, aes(x=x)) + 
  ...
  geom_text(x=1, y=0.25, label="+5", color="red") +
  geom_text(x=-2, y=0.25, label="0", color="black") +
  geom_text(x=-6, y=0.25, label="-5", color="blue") +
  labs(title="beta1 changes form", xlab="x", ylab="y") +
  JT.theme
