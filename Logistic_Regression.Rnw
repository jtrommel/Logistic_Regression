...
<<label=ROC5,fig=TRUE,include=FALSE, echo=FALSE>>=
pr <- prediction(preds, data.df[,2])
par(mfrow=c(2,2))
plot(performance(pr, measure = "tpr", x.measure = "fpr"))
plot(performance(pr, measure = "prec", x.measure = "rec"))
plot(performance(pr, measure = "sens", x.measure = "spec"))
plot(performance(pr, measure = "lift", x.measure = "rpp"))
par(mfrow=c(1,1))
...
\caption{Plots using package \textbf{ROCR}}
\label{fig:ROC5}
\setfloatalignment{b}% forces caption to be bottom-aligned
\end{figure}
