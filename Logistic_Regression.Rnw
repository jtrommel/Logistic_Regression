...
\begin{itemize}
  ...
  \begin{equation}
  BIC=-2ln\mathcal{L_{a}} + k.ln(n)=D_{a}+k.ln(n)
  \end{equation}
\end{itemize}
...
paste("number TP=",sum(comp.lr$group.name == "TP"))
paste("number FP=",sum(comp.lr$group.name == "FP"))
paste("number FN=",sum(comp.lr$group.name == "FN"))
paste("number TN=",sum(comp.lr$group.name == "TN"))
...
We are trying to get $Q$ as low as possible. In this case $Q=\Sexpr{round((confusion_matrix[1,2] + confusion_matrix[2,1])/n,4)}$.
...
for (i in seq(1:100)){
co_influence$Q[i] <- (table(data.df[,2],predict(mod.lr, type="response")>co_influence$co[i])[1,2] + table(data.df[,2],predict(mod.lr, type="response")>co_influence$co[i])[2,1])/n 
}
...
In view of the fact that the consequence of a failing o-ring could be (and in reality were) disastrous, we clearly do not want a model that leads to a classification where we predict ''Safe" (which in this case is coded as ''0"), while the reality is that it ''Fails" (coded as a ''1"). We do not want a False Negative. A False Positive means that the model classifies the situation as ''Fails" (coded as ''1"), while the reality is ''Safe" (coded as ''0"). This is annoying (the launch is delayed until the temperature is higher) and it will cost money, but these consequences are incomparable with the loss of life. We should therefore choose $co$ to minimize the False Negatives, even at the cost of higher False Positives. We can see this is a \emph{Receiver Operating Characteristic} (Figure~\ref{fig:ROC}).

<<label=ROC,fig=TRUE,include=FALSE, echo=FALSE>>=
co_influence <- data.frame(co=seq(0.1, 0.9, by=0.05), FP=0, TP=0)
for (i in seq(1:nrow(co_influence))){
co_influence$FP[i] <- (table(data.df[,2],predict(mod.lr, type="response")>co_influence$co[i])[1,2])/n
co_influence$TP[i] <- (table(data.df[,2],predict(mod.lr, type="response")>co_influence$co[i])[2,1])/n
}
ggplot(data=co_influence) +
  geom_line(aes(x=FP, y=TP)) +
  geom_point(aes(x=FP, y=TP)) +
  geom_text(aes(x=FP, y=TP, label=co), hjust=0.2, vjust=1) +
...
A choice of the cut-off point equal to 0.2 seems appropriate.
